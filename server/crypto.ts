...
const kmsClient = process.env.AWS_REGION ? new KMSClient({ region: process.env.AWS_REGION }) : null;
...
export async function decryptKms(encryptedBase64: string): Promise<string> {
  if (!kmsClient || !AWS_KMS_KEY_ID) {
  ...
  try {
    ...
    if (!response.Plaintext) throw new Error("KMS Decrypt failed: no plaintext");
    return Buffer.from(response.Plaintext).toString("utf-8");
  } catch (err: any) {
    console.warn("[KMS] Falling back to local decryption:", err.message);
    return decrypt(encryptedBase64);
  }
}

export async function encryptKms(text: string): Promise<string> {
  if (!kmsClient || !AWS_KMS_KEY_ID) {
    return encrypt(text);
  }
